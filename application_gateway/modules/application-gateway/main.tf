...
locals {
  frontend_port_name             = "appgw-${var.app_gw_name}-${var.region}-feport"
  frontend_ip_configuration_name = "appgw-${var.app_gw_name}-${var.region}-feip"
  gateway_ip_configuration_name  = "appgw-${var.app_gw_name}-${var.region}-gwipc"
...
resource "azurerm_public_ip" "pip" {
  name                = lower("${var.app_gw_name}-${var.region}-gw-pip")
  location            = var.region
  resource_group_name = var.resource_group
  allocation_method   = var.sku.tier == "Standard" ? "Dynamic" : "Static"
  sku                 = var.sku.tier == "Standard" ? "Basic" : "Standard"
  domain_name_label   = var.domain_name_label
  tags                = merge({ "ResourceName" = lower("${var.app_gw_name}-${var.region}-gw-pip") }, var.tags, )
}
...
resource "azurerm_application_gateway" "main" {
  name                = lower("appgw-${var.app_gw_name}-${var.region}")
  resource_group_name = var.resource_group
  location            = var.region
  enable_http2        = var.enable_http2
  zones               = var.zones
  firewall_policy_id  = var.firewall_policy_id != null ? var.firewall_policy_id : null
  tags                = merge({ "ResourceName" = lower("appgw-${var.app_gw_name}-${var.region}") }, var.tags, )
  ...
  frontend_ip_configuration {
    ...
    subnet_id                     = var.private_ip_address
  }
...
resource "azurerm_monitor_diagnostic_setting" "pip-diag" {
  name                       = lower("pip-${var.app_gw_name}-diag")
...
resource "azurerm_monitor_diagnostic_setting" "agw-diag" {
  name                       = lower("agw-${var.app_gw_name}-diag")
